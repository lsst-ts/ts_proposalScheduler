...
class TestAreaDistributionProposal(unittest.TestCase):
    ...
    def setUp(self):
        ...
        configfilepath = os.path.join(getPackageDir('ts_proposalScheduler'), 'tests', 'conf', 'survey', 'areaProp1.conf')
    ...
    def test_build_fields_tonight_list(self):
        ...
        configfilepath = os.path.join(getPackageDir('ts_proposalScheduler'), 'tests', 'conf', 'survey', 'weak_lensing.conf')
        (path, name_ext) = os.path.split(configfilepath)
        (name, ext) = os.path.splitext(name_ext)
        proposal_confdict = read_conf_file(configfilepath)
